...
class TV_w_Loss(nn.Module):
    def __init__(self):
        super(TV_w_Loss, self).__init__()

    def forward(self, x):
        [w, h, b] = x.shape
        w_tv =torch.abs(x[:w-1, :, :] - x[1:w, :, :])
        return w_tv.detach().clone()
...
class TVLoss(nn.Module):
    ...
    def forward(self, x):
        [s, w, t] = x.shape
        s_tv = torch.abs(x[:s-1, :, :] - x[1:s, :, :])
        t_tv =torch.abs(x[:, :, :t-1] - x[:, :, 1:t])
        w_tv = torch.abs(x[:, :w-1, :] - x[:, 1:w, :])
        return t_tv.sum().detach().clone() + w_tv.sum().detach().clone() + s_tv.sum().detach().clone()

class TV_b_Loss(nn.Module):
    def __init__(self):
        super(TV_b_Loss, self).__init__()

    def forward(self, x):
        [w, t] = x.shape
        t_tv = torch.abs(x[:w-1, :] - x[1:w, :])
        return t_tv.sum().detach().clone()

class Nuclear_Loss(nn.Module):
    def __init__(self):
        super(Nuclear_Loss, self).__init__()

    def forward(self, x):
        [w, h, b] = x.shape
        nuc = 0
        for i in range(b):
            nuc = nuc + x[:, :, i].norm('nuc')
        return nuc.detach().clone()




class TV_h_Loss(nn.Module):
    def __init__(self):
        super(TV_h_Loss, self).__init__()

    def forward(self, x):
        [w, h, b] = x.shape
        h_tv = torch.abs(x[:, :h-1, :] - x[:, 1:h, :])
        return h_tv.detach().clone()
